...
public class PerfilInvocador extends JFrame {
	...
	private void InicializarComponentes() {
		...
		int partidaYPos = 0;
		// ----------- historico partidas
		for (Partida partida : invocador.getPartidas()) {
			...
			String champNome = partida.getNomeChampPartida().replaceAll(" ", "");
			champNome = champNome.replace("'", "");
			champNome = champNome.replace(".", "");
			uri = "src/assets/champion/" + champNome + ".png";
			JLabel lblImgChamp = new JLabel(GetImage(uri, 120, 120));
			lblImgChamp.setLocation(5, 5);
			lblImgChamp.setSize(120, 120);
			
			uri = "src/assets/summonerspell/" + partida.getpSpellPartida() + ".png";
			JLabel lblImgFeitico1 = new JLabel(GetImage(uri, 45, 45));
			lblImgFeitico1.setLocation(135, 15);
			lblImgFeitico1.setSize(45, 45);
			
			uri = "src/assets/summonerspell/" + partida.getsSpellPartida() + ".png";
			JLabel lblImgFeitico2 = new JLabel(GetImage(uri, 45, 45));
			lblImgFeitico2.setLocation(135, 70);
			lblImgFeitico2.setSize(45, 45);

			JLabel lblNomeChamp = new JLabel(partida.getNomeChampPartida());
			...
			JLabel lblLvlChamp = new JLabel("Lvl " + partida.getLvlChamPartida());
			...
			String kda = partida.getKills() + "/" + partida.getDeaths() + "/" + partida.getAssists();
			...
			String status = "";
			if (partida.getStatusPartida() == true) {
				status = "Vitória";
			}else {
				status = "Derrota";
			}
			JLabel lblResultado = new JLabel(status);
			...
			JLabel lblTempo = new JLabel(ConverterTempo(partida.getTempoPartida()));
	...
	private String ConverterTempo(Long tempo) {
		String strTempo = "";
		int min = Math.toIntExact(tempo / 60);
		int seg = Math.toIntExact(tempo % 60);
		strTempo = ""+seg;
		if (strTempo.length() == 1) {
			strTempo = 0 + strTempo;
		}
		
		strTempo = min + ":" + strTempo;
